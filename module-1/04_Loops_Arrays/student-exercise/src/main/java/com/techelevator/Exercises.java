...
public class Exercises {
	...
	public int[] rotateLeft3(int[] nums) {
		for (int i=0; i<nums.length; i++) {
			if ( i == nums.length-1 ) { //last index of array
				nums[nums.length-1] = nums[0];//assign last index in new array to first index of previous array
			} else {
				nums[i]= nums[i+1];//assign current index to value of next index
			}
		}
		return new int[] {};
	}
	...
	public int[] reverse3(int[] nums) {
		int[] reverseArray = new int[] { nums[2], nums[1], nums[0]};		
		return reverseArray;
	}
	...
	public int[] middleWay(int[] a, int[] b) {
		int[] newArrayMid = new int[] { a[1], b[1]};
		return newArrayMid;
	}
